...
> [!WARNING]
> The `ai-sdlc-portal-prototype` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `ai-sdlc-portal-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag ai-sdlc-portal-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 ai-sdlc-portal-prototype:development
```
...
```bash
docker build --no-cache --tag ai-sdlc-portal-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 ai-sdlc-portal-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash ai-sdlc-portal-prototype
```
